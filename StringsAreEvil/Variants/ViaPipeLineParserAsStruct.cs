...
{
    public sealed class ViaPipeLineParserAsStruct<TLineParser> : Variant where TLineParser : ILineParser
    {
        private const byte NewLine = (byte)'\n';
        private readonly TLineParser _parser;

        public ViaPipeLineParserAsStruct(TLineParser lineParser) : base(lineParser)
        {
            _parser = lineParser;
        }
        ...
        private void ParseLinesFast(ReadOnlySpan<byte> span, out int consumed)
        {
            ...
            while (!span.IsEmpty)
            {
                ...
                if (index != -1)
                {
                    _parser.ParseLine(span.Slice(0, index));
        ...
        private void ParseLine(in ReadOnlySequence<byte> line)
        {
            ...
            {
                _parser.ParseLine(line.FirstSpan);
            }
            else
            {
        ...
        private void ParseLineMultiSegment(in ReadOnlySequence<byte> line)
        {
            ...
            else
            {
                ...
                {
                    line.CopyTo(buffer);
                    _parser.ParseLine(buffer.AsSpan(0, length));
                }
                finally
                {
